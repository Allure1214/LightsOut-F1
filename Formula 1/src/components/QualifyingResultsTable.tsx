...
function getPositionColor(position: number): string {
  if (position === 1) return 'bg-gradient-to-r from-purple-400 to-purple-600 text-white'
  ...
  if (position <= 10) return 'bg-purple-100 text-purple-800'
  return 'bg-gray-100 text-gray-800'
}

function getSessionColor(session: 'Q1' | 'Q2' | 'Q3'): string {
  switch (session) {
    case 'Q1': return 'text-red-600'
    case 'Q2': return 'text-yellow-600'
    case 'Q3': return 'text-green-600'
    ...
    <div className="space-y-4 qualifying-results-table">
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-semibold text-purple-800">Qualifying Format</span>
        </div>
        <div className="text-sm text-purple-700">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <div><span className="font-medium">Q1:</span> 20 drivers → 15 advance</div>
            <div><span className="font-medium">Q2:</span> 15 drivers → 10 advance</div>
            <div><span className="font-medium">Q3:</span> 10 drivers → Final grid order</div>
          </div>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b">
              ...
              <th className="text-center py-3 px-4 font-semibold text-muted-foreground">Q1</th>
              <th className="text-center py-3 px-4 font-semibold text-muted-foreground">Q2</th>
              <th className="text-center py-3 px-4 font-semibold text-muted-foreground">Q3</th>
              ...
              <tr key={index} className="border-b hover:bg-muted/50 transition-colors">
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
                    ...
                    {result.position === 1 && (
                      <Award className="w-4 h-4 text-purple-600" aria-label="Pole Position" />
                    )}
                  </div>
                </td>
                <td className="py-3 px-4">
                ...
                <td className="py-3 px-4 text-center">
                  {result.q1 ? (
                    <span className="text-sm font-mono">{result.q1}</span>
                  ) : (
                ...
                <td className="py-3 px-4 text-center">
                  {result.q2 ? (
                    <span className="text-sm font-mono">{result.q2}</span>
                  ) : (
                    <span className="text-sm text-muted-foreground">-</span>
                  )}
                </td>
                <td className="py-3 px-4 text-center">
                  {result.q3 ? (
                    <span className="text-sm font-mono font-semibold text-green-600">{result.q3}</span>
                  ) : (
                    <span className="text-sm text-muted-foreground">-</span>
                  )}
                </td>
                <td className="py-3 px-4 text-center">
                  <span className="text-sm font-mono font-semibold text-purple-600">{result.bestTime}</span>
                </td>
                <td className="py-3 px-4 text-center">
                  <Badge 
                    variant="outline" 
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="text-right">
                  <div className="font-bold text-lg text-purple-600">{result.bestTime}</div>
